...
**Current state — v0.1.0, tagged.** All three repos exist locally with commits and GitHub remotes under the `markleft-lang` org — `markleft`, `tests`, and `.github`; `markleft-rs` is not among them and is not due until Phase 2. This repo holds `README.md`, `definition.md`, and `deltas.md`, plus six memos under `notes/` and the `.claude/` scaffolding. `definition.md` is a **full draft** rewritten for decision 20 — twelve sections and four appendices, the language in prose in §4 and the decisions with rationale in §5; Appendix D lists fourteen open points, four answered for the first time and needing confirmation, three raised by decision 20. `deltas.md` is still a **scaffold**: the two delta tables are complete as to which decisions diverge, the per-delta detail is not written. `tests` has its `examples/` and `corpus/` directories awaiting content. No build, lint, or test command exists; add a Commands section here the moment `markleft-rs` lands, and make sure it covers running a *single* conformance example — that is the unit of debugging.
...
1. **Review and settle `definition.md`** — the full draft is written and rewritten for decision 20. What remains is the author's review, and closing Appendix D: items 1–4 are answered for the first time and need confirming (invalid UTF-8, leading BOM, code-point identity for matching, code-point count for "column"); items 5–14 are unanswered except D.6, closed by decision 19. The three newest are D.12 (may a brace group span a line ending), D.13 (is `**{…}` worth having), and D.14 (strikethrough and HTML entity references, both absent by accident). Anything confirmed goes into `.claude/decisions.md` as an amendment, not silently into the prose.

2. **Seed `tests/corpus/`** — original prose written to attack invariant 1. Decision 20 changed what the corpus should attack: the old targets (bare dollars, `snake_case`, `5 * 3 = 15`, file globs) are now safe by construction and worth one document each as regression evidence. The live targets are the **doublet openers** — `#{` in Ruby and Elixir interpolation, `*{` in shell brace expansion, `\{` in TeX — plus the six block markers at line start, which are where the remaining severity 2 and 3 collisions all live. Contributor-authored only.
...
4. **Update the `.github` status board and the `tests` repo** for decision 20. The org profile restates the roadmap and is the first thing a visitor reads; `tests` records the spec revision it targets, and that revision is now 0.1.0.
