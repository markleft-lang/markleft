...
23. **Asides are core: `<` is the converse of `>`** — a line beginning `<` then a required space is a line of an aside, `<` alone its blank line; a container nesting and holding any blocks, the block quote mirrored. Plain-text semantics: `>` presses harder than prose (quoted matter, standing out), `<` presses less (liftable with little impact) — meaning stated in the source, the test decision 19's thematic break failed. **Rendering open by design: sidebar, margin note, smaller type, or a disclosure fold — folding is one rendering, not the construct, so the mark means something in print**; retires decision 7's `<details>` casualty natively while the raw-HTML hole stays closed. Amended same-day to complete the mirror: **the space after `>` is required too** — the language's last optional-space clause deleted — so `<div>`/`<!DOCTYPE` and `>=`/`>>log`/`>5ms` at line start are all text by construction; nesting is written `> > ` (aligned, breathing), and the migrator rewrites `>text` and `>>` mechanically. No title line, no type vocabulary (`< note` would be a class — decision 9; "Note:" is text). Ledger: `<` respent at severity 1 a day after decision 20 freed it; fifteen of thirty-two free; **the line-start column grows to seven** — the six-marker sentence in decisions 20–22 is superseded knowingly.

24. **One source line per block — recommended, never required** *(non-normative)*: write a paragraph or list item as one line and let renderers soft-wrap. Diff hygiene (reflow buries the edit — the rule `.claude/markdown-conventions.md` already binds this repo to, generalized into the language's guidance) plus prose-safety: line start is the most reserved position, and a hard wrap can open a continuation line with `- `, `> `, or `1984. ` and fire a marker mid-paragraph, where a single-line paragraph exposes only its true opening. The formatter writes it; wrapped documents stay well-formed — a recommendation in grammar's clothing would make whitespace structural against decision 10's spirit. Shrinks D.12's blast radius and Finding 1's everyday pressure without deciding either.

25. **Comments are core: `//` opens a comment line** — a run of exactly two `/` at line start, then one space (or `//` alone); consecutive lines are one comment block; content uninterpreted and **not document content**. Carries the rider on decision 15: a comment is the cardinal rule's mirror image (source that does not render), lawful because comment text is addressed to the author — **the document is the source minus its comments**; the plain-text reader still reads everything. Blocks only, on principle: a mid-line to-EOL form fires unconditionally (eats quoted poetry/code silently) or by flanking (banned, decision 20), and under decision 24 "to end of line" would mean "to end of paragraph". The ceiling fallback does the safety work — `/usr/bin`, `///`, `//cdn.example.org` all stay text; the one collision is a pasted C-family comment line outside a fence, which vanishes (the first construct whose collision failure is silence — severity 1, narrow trigger). Not adopted: `%%` (the runner-up — better vanish distribution and ledger, lost on reading experience), `%`, `;`, `#`, `<!-- -->`. Ledger: `/` spent at severity 1, line start only — free inside lines, so paths and URLs untouched; fourteen of thirty-two free; **line-start column grows to eight**. Migrator rewrites `<!-- … -->` mechanically.
