...
1. **Review and settle `definition.md`** — the full draft is written (twelve sections, four appendices, public-standard voice). What remains is the author's review, and closing Appendix D: items 1–4 are answered there for the first time and need confirming (invalid UTF-8, leading BOM, code-point identity for matching, code-point count for "column"); items 5–11 are unanswered except D.6, closed by decision 19 (remaining: tilde fences, lazy continuation in block quotes, the loose/tight rule, verbatim-span whitespace stripping, media type, and what a line with a malformed decorator list becomes). Anything confirmed goes into `.claude/decisions.md` as an amendment, not silently into the prose.
