...
**2026-08-07 — the cardinal rule promoted to invariant 5.** §3 now carries five invariants; `CLAUDE.md`, `charter.md` §3, and `README.md` follow. Decision 15 keeps the operative detail and the worked consequences.
...
- **Stated as enabling, not restrictive** — the framing matters and is now in §3, `CLAUDE.md`, `charter.md`, and `README.md`. The rule is the minimal set that lets capability live in tooling and be built cleanly. Features are **relocated, not forbidden**: they move to where they can be implemented against a format that never moves (§2). Without the rule there is no guarantee for tooling to rely on; with it, a tool knows exactly what the source will still say after it writes into it.

- **`README.md` now lists six guarantees**, the new one user-facing as *"Read it anywhere"*. `charter.md` §3 must explain the five-versus-six relationship rather than leave it to inference.
...
- **Third appearance of the same argument** — hard wrap, ordered-list numerals, now anchor labels. *Do not encode a position in something that other things point at*, because positions shift and references do not want to. Worth stating once in the charter rather than three times in passing.
