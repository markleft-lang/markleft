...
*Origin conversation: August 2026. Status: pre-definition. This document records the decisions, rationale, and open items from the founding discussion.*
...
2. **Underscore is dead as emphasis syntax.** *(Scope clarified 2026-08-08 — decision 18.)* Emphasis is `*em*` / `**strong**` only. Bracketed form (djot-style `{*...*}`) for intra-word cases. Flanking rules collapse to two lines. The heading read "dead as syntax" until decision 18 gave `_` one position, immediately before `{`; what this decision argued for is untouched, since a subscript needs a brace after the sigil and `snake_case` still takes no escape.
...
18. **Superscript and subscript are core, braced, and typographic only.** *(Decided 2026-08-08. Rule: R29.)* `^{…}` raises inline content and `_{…}` lowers it. **The sigil carries meaning only when the very next code point is `{`**, which is the whole of the prose-safety argument: `2^32`, `[^0-9]`, `ISO_8601`, `file_2.txt`, `UTF_8`, and `snake_case` are all still text, needing no escape, because none of them puts a brace after the sigil. **What the construct does is a baseline offset and a size reduction, and nothing else** — no italics, no math spacing, no symbol interpretation. That is the definition rather than a restriction placed on top of it.
    ...
    **Why the sigil sits outside the brace,** against the language's own brace-group family. `{^2}` was the first proposal and it is cheaper on the reference card: R22 `{*em*}` and R23 `{#id}` already make `{` plus a sigil one shape, four constructs would share one rule, and it moves no character out of the free column at all. It was set aside on **invariant 5**. `H_{2}O` is a plain-text reading convention scientists have had for decades; `H{_2}O` makes a plain-text reader stop. The cardinal rule is a promise to the reader, so where the two arguments meet, the reader's is the one that governs. Consistency lost to legibility, deliberately and once.
    ...
    **The price, stated where it can be seen.** `^` leaves the free column of the reserved-position table — the first decision to move a character that way since the table was written — and `_` gains a meaning inside a line. Both land at severity 1. Two new collisions: raw TeX pasted into prose outside a fence (`\int_{0}^{\infty}`), which R17 already mangled before this rule existed and whose answer is unchanged, and `{{^inverted}}` in Mustache and Handlebars, billed to the audience already paying for `{#if}` under decision 17. Set against that, `_` comes out **safer than CommonMark's**, where underscore is emphasis and identifiers italicize mid-word under the flanking rules — a rare direction of travel worth noting, since the language adds a construct and the character gets safer than the baseline.

    **The accepted failure mode.** Someone with TeX fingers writes `E=mc^2` and gets a literal caret. That failure is *visible* on the page rather than silent, which is the class this project tolerates — the same class as decision 5's over-long closing fence — and the validator catches it at the point of failure with a diagnostic on a sigil followed by a bare alphanumeric run, which is the shape Finding 3 already proposes for the backslash.
...
19. **The thematic break is removed.** *(Decided 2026-08-08. R13 struck.)* A line of three or more `-`, `_`, or `*` reserves nothing. Such a line is ordinary paragraph text, or a bullet item where it takes that form (`- - -`). **The argument is that the construct never had a plain-text meaning to preserve** — and this language's whole method is to ask what a mark means to someone reading the source with no renderer. A run of hyphens on its own line says "something changes here", and what changes is never stated: a topic shift, a scene break, an aside ending, the end of the document. A heading says which. So the construct fails invariant 5 from the inside: it is not that a reader cannot see it, but that seeing it tells them nothing.
    ...
    **What the removal returns.** `*` and `_` each get their **line-start column back**, the first dividend of that kind since `+`. Neither becomes free outright, both keeping inline jobs, so the free-column count is unchanged; what changes is that the characters claiming the first column are now exactly `` ` ``, `#`, `-`, digits, `>`, `|`, `\`, and `[`. **And R8 loses its only exception** — the clause making a thematic break win over a bullet item on `- - -` was the sole precedence contest in the block layer, and an ordering exception is an "unless" clause in different clothing, so invariant 2 collects a dividend that has nothing to do with rendering. `-` is untouched at severity 3, since R8 still claims it.
...
- **Phase 3 — Tooling.** Validator with real diagnostics ("unclosed emphasis opened at 12:4"); canonical formatter (gofmt-style, one serialization per AST); CommonMark→Markleft migrator with change report (the adoption bridge).
...
- **`text` names the default explicitly**, idempotent with the bare fence and bare span. Two spellings of one meaning is fine — invariant 3 rules out one spelling with two parses, not the reverse — and saying "this is plain text" out loud is the whole point of a decorator.
