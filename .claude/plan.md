...
- **Fold decisions 21 and 22 into the definition.** Task lists (`- [x]`) and strikethrough (`~{…}`), both adopted 2026-08-10, postdate the definition draft, so `definition.md` §4 and §5 and the reference card do not mention them yet. Add both constructs during the settling pass, and give the corpus a target for each new collision — a bullet item quoting a literal `[x]` box, and shell tilde-and-brace expansion (`~{alice,bob}/bin`) quoted in prose.
