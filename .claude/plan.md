...
- **Fold decisions 21, 22, and 23 into the definition.** Task lists (`- [x]`), strikethrough (`~{…}`), and asides (`< `), all adopted 2026-08-10, postdate the definition draft, so `definition.md` §4 and §5 and the reference card do not mention them yet. Add the three constructs during the settling pass — the aside also amends §4.3.7's closed block-opener list — and give the corpus a target for each new collision: a bullet item quoting a literal `[x]` box, shell tilde-and-brace expansion (`~{alice,bob}/bin`) quoted in prose, and a technical paragraph opening with a spaced comparison (`< 5 ms`).
